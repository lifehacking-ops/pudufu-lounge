...
(function () {
  ...
  var POSTS = D.posts;
  /* 목업 글에는 id 가 없다. 주소(?p=)와 뒤로가기가 글 하나를 가리키려면 이름이 있어야
     한다. 서버 글은 항상 id 를 갖고 오므로 앱에서는 아무 일도 안 한다. */
  POSTS.forEach(function (p, i) { if (p.id == null) p.id = "m" + (i + 1); });
  ...
  function renderSecMenu(cur) {
    ...
    if (old) old.remove();
    var menu = el("div", "pick-menu cat-pop sec-menu");
    menu.hidden = true;
